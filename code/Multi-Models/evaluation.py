...
AIHUBMIX_CHAT_COMPLETIONS_URL = "https://aihubmix.com/v1/chat/completions"
FORMAL_EVALUATION_PROVIDER = "aihubmix-gpt-4.1-free"
DEFAULT_MPC_FUNCTIONAL_GROUP_CACHE = (
    Path("results")
    / "Multi-Models"
    / "shared_cache"
    / "gpt-4.1-free_functional_group_cache.json"
)
MPC_FUNCTIONAL_GROUP_PROMPT_VERSION = "official-vocabulary-json-v1"
REQUEST_INTERVAL_SECONDS = 0.0
LAST_API_ATTEMPT_MONOTONIC: float | None = None
...
LLM_PROVIDERS = {
    ...
    "aihubmix-coding-glm-4.7-free": {
        "key_env": "AIHUBMIX_CODING_GLM_4_7_FREE_API_KEY",
        "default_model": "coding-glm-4.7-free",
        "base_url": AIHUBMIX_CHAT_COMPLETIONS_URL,
    },
    "aihubmix-gpt-4.1-free": {
        "key_env": "AIHUBMIX_GPT_4_1_FREE_API_KEY",
        "default_model": "gpt-4.1-free",
        "base_url": AIHUBMIX_CHAT_COMPLETIONS_URL,
    },
    "aihubmix-xiaomi-mimo-v2.5-free": {
        "key_env": "AIHUBMIX_XIAOMI_MIMO_V2_5_FREE_API_KEY",
        "default_model": "xiaomi-mimo-v2.5-free",
        "base_url": AIHUBMIX_CHAT_COMPLETIONS_URL,
    },
}
...
def post_chat_payload(payload: dict[str, Any], llm_config: dict[str, str]) -> dict[str, Any]:
    ...
    global LAST_API_ATTEMPT_MONOTONIC
    ...
    for attempt in range(3):
        try:
            if LAST_API_ATTEMPT_MONOTONIC is not None:
                elapsed = time.monotonic() - LAST_API_ATTEMPT_MONOTONIC
                remaining = REQUEST_INTERVAL_SECONDS - elapsed
                if remaining > 0:
                    time.sleep(remaining)
            LAST_API_ATTEMPT_MONOTONIC = time.monotonic()
    ...
    while True:
        ...
        try:
            ...
            if use_temperature and is_temperature_compat_error(exc):
                ...
                continue
            if exc.status == 429 and REQUEST_INTERVAL_SECONDS > 0:
                print(
                    "EVALUATION_PROGRESS rate_limited; retrying the same request "
                    f"after the {REQUEST_INTERVAL_SECONDS:g}-second request interval",
                    flush=True,
                )
                continue
            raise
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="minimum interval between evaluation API attempts; defaults to 60 seconds",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.request_interval_seconds < 0:
            raise EvaluationError("--request-interval-seconds must be non-negative")
